...
import { ChallengePage, type FormRowData } from '../pages/ChallengePage.js';
...
const exactExpectedErrorMessage =
  'We didn’t find an account with those login credentials';

test.describe('Automation Challenge', () => {
  test('should successfully login and complete the end-to-end automation challenge using Excel data', async ({
    page,
  }) => {
    if (!validUsername || !validPassword) {
      logger.warn(
        'TEST_USERNAME or TEST_PASSWORD not set in .env. Skipping login test.',
      );
    ...
    await Promise.race([dialogPromise, page.waitForTimeout(2500)]);

    if (dialogMessage !== null) {
      if (dialogMessage === exactExpectedErrorMessage) {
        test.fail(
          true,
          `Login attempt with credentials ("${validUsername}") failed as the site reported the expected error dialog: "${dialogMessage}". If credentials were valid, this is unexpected.`,
        );
        logger.error(
          `Login with credentials ("${validUsername}") failed as expected for invalid ones. Dialog: "${dialogMessage}"`,
        );
      } else {
        // If the dialog is not exactly as expected, it's a different kind of failure.
        test.fail(
          true,
          `Login attempt resulted in an UNEXPECTED dialog message. Received: "${dialogMessage}". Expected: "${exactExpectedErrorMessage}".`,
        );
        logger.error(
          `Unexpected dialog message. Received: "${dialogMessage}". Expected: "${exactExpectedErrorMessage}"`,
        );
      }
      return;
    } else {
      logger.info(
        'No dialog appeared. Proceeding with successful login verifications.',
      );
      await expect(
        page.getByRole('button', { name: 'Alejo' }),
        'User button with text "Alejo" should be visible after successful login',
      ...
      await expect(
        startButton,
        'Start button should be visible after login',
      ...
      // Calculate current directory for ES modules
      ...
      try {
        ...
        logger.error(
          `Failed to read Excel data from "${excelFilePath}", sheet "${sheetName}":`,
          { error: (error as Error).message, stack: (error as Error).stack },
        );
        test.fail(
          true,
          `Critical error: Could not read test data from Excel. ${(error as Error).message}`,
        );
        return;
      }
      ...
      if (!allRowsData || allRowsData.length === 0) {
        logger.warn(
          'No data found in Excel file or sheet. Skipping form submissions.',
        );
      ...
      for (const [index, rowData] of allRowsData.entries()) {
        ...
        try {
          ...
          logger.error(
            `Error processing row ${index + 1} (Data: ${JSON.stringify(rowData)}):`,
            { error: (error as Error).message, stack: (error as Error).stack },
          );
          test.fail(
            true,
            `Failed to process form for row ${index + 1}: ${(error as Error).message}`,
          );
          break;
        }
      }
